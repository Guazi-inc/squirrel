...
func (eq Eq) toSql(useNotOpr bool) (sql string, args []interface{}) {
	var (
		exprs      []string
		equalOpr   = "="
		inOpr      = "IN"
...
func (lt Lt) toSql(opposite, orEq bool) (sql string, args []interface{}) {
	var (
		exprs []string
		opr   string = "<"
	)
...
func (c conj) join(sep string) (sql string, args []interface{}) {
	var sqlParts []string
	for _, sqlizer := range c {
		partSql, partArgs:= sqlizer.ToSql()
		if partSql != "" {
